            ...
            <div className={'h-56 w-56 mr-16 '}>
                ...
                {inp && <div className={'absolute opacity-0'}>
                    <input onBlur={() => {
                        ...
                        clearTimeout(timer)
                        setFlag(true)
                        if (e.keyCode === 37 && direction.x !== 1 && direction.y !== 0) {
                                setDirection(directions.left)
                        } else {
                            setGo(go + 1)
                        }
                        if (e.keyCode === 39 && direction.x !== -1 && direction.y !== 0) {
                                setDirection(directions.right)
                        } else {
                            setGo(go + 1)
                        }
                        if (e.keyCode === 38 && direction.x !== 0 && direction.y !== 1) {
                                setDirection(directions.up)
                        } else {
                            setGo(go + 1)
                        }
                        if (e.keyCode === 40 && direction.x !== 0 && direction.y !== -1) {
